...
async def _run_unsloth_finetuning_job(
    ...
    # Create callback for saving model after each epoch
    epoch_checkpoint_callback = None
    if job.local_output_path is not None:
        from transformers import TrainerCallback
        from pathlib import Path
        
        class EpochCheckpointCallback(TrainerCallback):
            def __init__(self, model, tokenizer, base_output_path):
            ...
            def on_epoch_end(self, args, state, control, **kwargs):
                ...
                checkpoint_path = Path(str(self.base_output_path) + f"_epoch_{epoch}")
                checkpoint_path.mkdir(parents=True, exist_ok=True)
                
                logger.info(f"Saving merged checkpoint after epoch {epoch} to {checkpoint_path}...")
                ...
                logger.success(f"Epoch {epoch} checkpoint saved to {checkpoint_path}")
        
        epoch_checkpoint_callback = EpochCheckpointCallback(
            model, tokenizer, job.local_output_path
        )
    
    callbacks = [epoch_checkpoint_callback] if epoch_checkpoint_callback else []
    ...
    # Save model locally or upload to HuggingFace
    if job.local_output_path is not None:
        id = hf_driver.save_local(job.local_output_path, model, tokenizer)
        logger.info(f"Model saved locally to: {id}")
    else:
        id = hf_driver.push(job.hf_model_name, model, tokenizer)
        logger.info(f"Model uploaded to HuggingFace: {id}")
