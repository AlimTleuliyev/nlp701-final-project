...
def cleanup_llm():
    """Destroy the VLLM instance and free GPU memory"""
    global _LLM
    if _LLM is not None:
        # Shutdown the engine and workers
        try:
            _LLM.llm_engine.shutdown()
        except Exception:
            pass  # Sometimes already shut down
        del _LLM
        _LLM = None
...
def _build_lora_request(model_id: str) -> LoRARequest:
    ...
    if Path(model_id).exists():
        ...
        # It's a HuggingFace repo, download it
